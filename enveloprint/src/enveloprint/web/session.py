...
__all__ = [
    ...
    'Store', 'DiskStore', 'DBStore', 'GoogleStore',
]
...
web.config.session_parameters = utils.storage({
    ...
    'cookie_domain': None,
    'timeout': 3600, #24 * 60 * 60, # 24 hours in seconds
...
class Session(utils.ThreadedDict):
    ...
    def __init__(self, app, store, initializer=None, cache_handler=None):
        ...
        self.__dict__['_cache_handler'] = cache_handler
    ...
    def _check_expiry(self):
        ...
        if self.session_id and self.session_id not in self.store:
            if self._config.ignore_expiry:
                self.session_id = None
                web.debug("calling cache_handler")
                if self._cache_handler and hasattr(self._cache_handler, '__call__'):
                    self._cache_handler()
            else:
                return self.expired()
    ...
    def _save(self):
        ...
        if not self.get('_killed'):
            web.setcookie(cookie_name, self.session_id, expires=expires, domain=cookie_domain)
