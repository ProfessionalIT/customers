...
def get_publicacoes(tipo_publicacao):
    records = db.query("select n.titulo, n.slug, n.intro, n.data_hora, (select login from usuario where id = n.autor) as autor from publicacao n inner join publicacao f ON f.id = n.id AND f.status = 'A' inner join publicacao_tipo_publicacao p ON p.publicacao = n.id AND p.tipo_publicacao = (select t.id from tipo_publicacao t where t.titulo like '%(tipo_publicacao)s')" % {'tipo_publicacao': tipo_publicacao})
    return records or None
...
def get_publicacao(endereco):
    try:
        record = db.query("select n.titulo, n.slug, n.completa, n.data_hora, (select u.login from usuario u where u.id = n.autor) as autor from publicacao n where n.slug = '%(endereco)s'" % {'endereco': endereco})[0]
        return record
    except IndexError:
        return None
