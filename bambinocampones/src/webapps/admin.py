        ...
        try:
            if i.username and i.password:
                ...
                if logado.login:
                    ...
                    msg = 'O usuario realizou o LOGIN no Modulo Administrativo'
...
class Logout(object):
    def GET(self):
        msg = 'O usuario saiu do Modulo Administrativo'
...
class EntityList():
    ...
    def GET(self, param):
        ...
        return render.layout(render.listar(form,
                                           pagination,
...
class EntityEdit():
    ...
    def GET(self, param, record_id):
        ...
        if param == 'Imovel':
            ...
            entry.valor_condominio = moeda_brasileira(entry.valor_condominio,
                                                      False)
            entry.valor_agua = moeda_brasileira(entry.valor_agua, False)
            entry.valor_iptu = moeda_brasileira(entry.valor_iptu, False)
            frm = getattr(forms, 'get%sForm' % param)()
            frm.fill(entry)
            recursos = getattr(model, 'ImovelRecurso').\
                               get_recursos_by_imovel(entry.id)
            msg = 'O usuario editou o registro %(registro)s na '\
                  'Pagina %(pagina)s do Modulo ' \
                  'Administrativo' % {'registro': frm.d['id'],
                                      'pagina': param}
            getattr(model, 'Log').new_log({'usuario': get_logged_user().id,
                                           'operacao': 'EDITAR',
                                            'log': msg,
    ...
    def POST(self, param, record_id):
        if param == 'ImovelRecurso':
            frm = forms.getImovelRecursoForm(record_id)
        else:
            frm = getattr(forms, 'get%sForm' % param)()
        ...
        if frm.d.operation == 'delete':
            raise web.seeother('/excluir/%(entity)s/%(id)s/' % \
                               {'entity': param, 'id': record_id}
                              )
        elif frm.d.operation == 'save':
            getattr(model, param).save(frm.d)
            raise web.seeother('/editar/%(entity)s/%(id)s' % \
                               {'entity': param, 'id': record_id}
                              )
        else:
            getattr(model, param).save(frm.d)
