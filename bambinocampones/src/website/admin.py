...
import model
from model import AlreadyExistsValueException
        ...
        try:
            if i.username and i.password:
                ...
                if logado.login:
                    ...
                    getattr(model, 'Log').new_log({'usuario': logado.id,
                                                   'operacao': 'LOGIN',
                                                   'log': """O usuario realizou
                                           o LOGIN no Modulo Administrativo""",
...
class Logout(object):
    ...
    def GET(self):
        """
        """
        getattr(model, 'Log').new_log({'usuario': get_logged_user().id,
                                       'operacao': 'LOGOFF',
                                       'log': """O usuario saiu do Modulo
                                                 Administrativo""",
                                       'endereco_ip': web.ctx.ip,
                                       'menu': 'Home'})
...
class EntityList():
    ...
    def GET(self, param):
        ...
        request_vars = {'form': form,
                        ...
                        'access_control': CONTROL_CONCURRENT_ACCESS}
        if param == 'Moderacao':
...
class EntityInsert():
    ...
    def GET(self, param, alternate_id=None):
        ...
        display_message = web.cookies().get('display_message')
        web.setcookie('display_message', '')
        request_vars = {'form': frm,
                        'img_url': '/static/images/%s.png' % param,
                        'titulo': 'Incluir %s' %
    ...
    def POST(self, param, alternate_id=None):
        ...
        if frm.validates():
            ...
            except AlreadyExistsValueException, (instance):
            ...
            if param == 'Usuario':
                display_message = 'Inseriu o registro %s com sucesso !' % \
                                                                    frm.d.login
            elif param == 'Enquete':
                display_message = 'Inseriu o registro %s com sucesso !' % \
                                                                 frm.d.pergunta
            ...
            if frm.d.operation == 'save':
                raise web.seeother(WEBSITE_URL + '/admin' + '/inserir/%s' %
                                                                         param)
            else:
                raise web.seeother(WEBSITE_URL + '/admin' + '/listar/%s' %
                                                                         param)
        else:
            request_vars = {'form': frm,
                            'img_url': '/static/images/%s.png' % param,
...
class EntityEdit():
    ...
    def GET(self, param, record_id):
        ...
        if param == 'Publicacao':
            ...
            frm = getattr(forms, 'get%sForm' % param)()
        frm.fill(entry)
        action_url = '%(path)s/set-unlocked/%(entity)s/%(id)s' % \
                     {'path': str(web.ctx.home),
                      'entity': param,
                      'id': record_id}
        request_vars = {'form': frm,
                        'img_url': '/static/images/%s.png' % param,
                        'titulo': 'Alterar %s' %
                                        getattr(model, param).get_form_title(),
                        'verbo': 'editar', 'display_message': display_message,
    ...
    def POST(self, param, record_id):
        ...
        if param == 'Publicacao':
            ...
            if not frm.validates():
                ...
                return render.layout(render.form(request_vars))
        if param == 'Enquete':
            display_message = 'Alterou o registro %s com sucesso !' % \
                            frm.d.pergunta
        elif param == 'Usuario':
            display_message = 'Alterou o registro %s com sucesso !' % \
                            frm.d.login
        ...
        if frm.d.operation == 'delete':
            raise web.seeother(WEBSITE_URL +
                            '/admin' + '/excluir/%(entity)s/%(id)s/' %
                               {'entity': param, 'id': record_id})
        elif frm.d.operation == 'save':
            getattr(model, param).save(frm.d)
            raise web.seeother(WEBSITE_URL +
                            '/admin' + '/editar/%(entity)s/%(id)s' %
                               {'entity': param, 'id': record_id})
...
class EntityPreview():
    ...
    def GET(self, param, slug):
        return render_layout.layout('menu_home',
                                    render_layout.preview(param, slug))
