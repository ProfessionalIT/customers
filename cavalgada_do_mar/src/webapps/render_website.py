...
from model import get_active_template, get_menu_itens, get_pagina, get_pagina_preview, get_images_slideshow, get_publicacoes, get_publicacao
...
def layout(active_menu, where_i_am, content):
    template_vars = {'context_path': WEBSITE_URL, 'menu_itens': get_menu_itens(), 'active_menu': active_menu, 'where_i_am': where_i_am,  'banners': get_images_slideshow()}
    return get_layout_path().layout(template_vars, content)

def index():
    boletins = get_publicacoes('Boletim')[0]
    return get_layout_path().index(boletins)

def boletins(pagination):
    boletins = get_publicacoes('Boletim')
    return get_layout_path().publicacoes('boletim', 'Boletins', pagination)

def boletim(slug_boletim):
    boletim = get_publicacao(slug_boletim)
    return get_layout_path().publicacao(boletim)
