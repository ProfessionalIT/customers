...
def layout(content):
    ...
    if usuario:
        render_vars = {'website_name': WEBSITE_NAME, 'version': ADMIN_VERSION, 'user': get_logged_user(), 'logoff_url': get_logoff_url(), 'exposed_managed_tables': get_exposed_managed_tables(usuario.nivel_acesso)}
    else:
        render_vars = {'website_name': WEBSITE_NAME, 'version': ADMIN_VERSION, 'user': get_logged_user(), 'logoff_url': get_logoff_url(), 'exposed_managed_tables': []}
...
def form(request_vars):
    ...
    request_vars['action'] = ''
    if request_vars['in_moderation_mode'] == 'yes':
        request_vars['display_message'] = 'Em Moderação - Visualizando Registro !'
        return admin_template_path.moderate_form(request_vars)
    else:
...
def listar(request_vars):
    filtro = ''
    if request_vars['pagination'].query is not None and request_vars['pagination'].query != 'None' and request_vars['pagination'].query != 'all' and request_vars['pagination'].query != '':
...
"""
def moderar(frm, pagination, display_message=None):
    filtro = ''
    if pagination.query is not None and pagination.query != 'None' and pagination.query != 'all' and pagination.query != '':
        ...
        if query == "acao='A'":
            ...
            query = 'Refused'
        filtro = 'Filtrando por: %(query)s' % {'query': query}
    return admin_template_path.modera(frm, filtro, pagination, display_message)
"""
