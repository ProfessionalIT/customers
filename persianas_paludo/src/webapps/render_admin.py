...
def layout(content):
    render_vars = {'website_name': WEBSITE_NAME, 'version': ADMIN_VERSION, 'user': get_logged_user(), 'logoff_url': get_logoff_url(), 'exposed_managed_tables': get_exposed_managed_tables()}
    return admin_template_path.admin(render_vars, content)

def index_admin():
    return admin_template_path.index_admin()

def form(form, img_url=None, titulo=None, verbo=None, display_message=None, **kw):
    method = kw.pop('method', 'post')
    action = kw.pop('action', '')
    return admin_template_path.form(titulo, img_url, action, method, form, verbo, display_message)

def form_update(form, img_url, imovel_id, recursos, titulo=None, verbo=None, display_message=None, **kw):
    method = kw.pop('method', 'post')
    action = kw.pop('action', '')
    return admin_template_path.form_update(titulo, img_url, action, method, form, imovel_id, recursos, verbo, display_message)
