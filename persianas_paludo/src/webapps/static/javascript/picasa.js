    ...
    gallery: function (galleryObjectId, galleryObjectName, settings) {
        ...
        this.setup = function (data) {
            ...
            for (var x = 0; x < this.totalItems; x++) {
                linha = '<li><a href="#photo" onclick="set_picture(\'' + this.itemsArray[x].media$group.media$content[0].url+'\', \'' + this.itemsArray[x].media$group.media$description.$t + '\');"><img src=\'' + this.itemsArray[x].media$group.media$thumbnail[0].url+ '\' class="photo_icon" /></a></li>';
                jQuery('#' + this.containerId).append(linha);
            }
        };
    },
